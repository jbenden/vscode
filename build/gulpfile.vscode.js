...
const builtInExtensions = [
    { name: 'ms-vscode.node-debug', version: '1.10.20' },
...
function packageTask(platform, arch, opts) {
    ...
    return () => {
        ...
        if (process.platform !== 'freebsd') {
            ...
            sources = es.merge(src, extensions, marketplaceExtensions)
                .pipe(filter(['**', '!**/*.js.map']));
        } else {
            sources = es.merge(src, extensions)
                .pipe(filter(['**', '!**/*.js.map']));
        }
...
gulp.task('clean-vscode-freebsd-ia32', util.rimraf(path.join(buildRoot, 'VSCode-freebsd-ia32')));
gulp.task('clean-vscode-freebsd-x64', util.rimraf(path.join(buildRoot, 'VSCode-freebsd-x64')));
...
gulp.task('vscode-freebsd-ia32', ['optimize-vscode', 'clean-vscode-freebsd-ia32'], packageTask('freebsd', 'ia32'));
gulp.task('vscode-freebsd-x64', ['optimize-vscode', 'clean-vscode-freebsd-x64'], packageTask('freebsd', 'x64'));
...
gulp.task('vscode-freebsd-ia32-min', ['minify-vscode', 'clean-vscode-freebsd-ia32'], packageTask('freebsd', 'ia32', { minified: true }));
gulp.task('vscode-freebsd-x64-min', ['minify-vscode', 'clean-vscode-freebsd-x64'], packageTask('freebsd', 'x64', { minified: true }));
